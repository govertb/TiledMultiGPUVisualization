                       ...
                       float mx, float my)
{
    ...
    for (i = threadIdx.x + blockIdx.x * blockDim.x; i < nbodiesd; i += inc)
    {
        dx = mx - body_posd[i].x;
        dy = my - body_posd[i].y;
        float mouse_dist2 = dx*dx + dy*dy;

        // The different interactions
        if(mouse_repulse)
        {
            // add force away from (mx, my)
            fxd[i] -= dx / mouse_dist2 * mouse_mass * 2500;
            fyd[i] -= dy / mouse_dist2 * mouse_mass * 2500;
        }

        if(mouse_heat)
        {
            temp_offsetd[i] = (dx+dy) / mouse_dist2 * mouse_temp;
        }
                       ...
                       float *temp_offsetd)
{
    ...
    for (i = threadIdx.x + blockIdx.x * blockDim.x; i < nbodiesd; i += inc)
    {
        ...
        factor = global_speed / (1.0 + sqrtf(global_speed * swg)) + temp_offsetd[i];
